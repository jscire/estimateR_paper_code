...
INFERENCE_DIR <-  here::here("data", "epidemia")
...
## Summarize run time
total_runtime <- all_results %>% 
  filter(idx == 1) %>% 
  summarize(total = sum(run_time)) %>% 
  pull(total)
...
## Compute coverage, RMSE and summarize results over all replicates
