...
# Read all analyses results and merge them into a single dataframe
all_results <- list()
...
summary_computation <- coverage_computation %>% 
  group_by(idx, Rt_type, simulation_type, noise_type, bootstrapping_type) %>% 
  ...
  arrange(bootstrapping_type, Rt_type, simulation_type, noise_type, idx)
