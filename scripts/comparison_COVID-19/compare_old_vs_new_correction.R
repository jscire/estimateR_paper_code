...
Rt_values_linear_up <- smooth_incidence(c(rep(1.3, times = 75), seq(from = 0.3, to =  2, length.out = 75)), data_points_incl = 8)

Rt_values <- Rt_values_linear_up
...
Re_with_new_correction <- get_bootstrapped_estimates_from_combined_observations(
  partially_delayed_incidence = simulated_observations$partially_delayed,
  fully_delayed_incidence = simulated_observations$fully_delayed,
  delay_until_partial = delay_short,
  delay_until_final_report = delay_long,
  ...
  include_index = TRUE,
  data_points_incl = 9) %>% 
  mutate(Rt_type = "new_correction") 

Re_with_old_correction <- get_bootstrapped_estimates_from_combined_observations(
  partially_delayed_incidence = simulated_observations$partially_delayed,
  fully_delayed_incidence = simulated_observations$fully_delayed,
  delay_until_partial = delay_short,
  delay_until_final_report = delay_long,
  N_bootstrap_replicates  = 100,
  correct_before_smoothing = FALSE,
  combine_bootstrap_and_estimation_uncertainties = TRUE,
  output_Re_only = FALSE,
  include_index = TRUE,
  data_points_incl = 9) %>% 
  mutate(Rt_type = "old_correction") 

comparison_data <- bind_rows(Re_with_new_correction, Re_with_old_correction) %>% 
...
plot_comparison_data$`Re values` <- recode_factor(plot_comparison_data$Rt_type,
                                                new_correction= "estimateR",
                                                old_correction= "Huisman et al. pipeline")

write_csv(plot_comparison_data, file = file.path(OUT_DIR, "plot_data_correction_old_vs_new_autocorrelated.csv"))
plot_comparison_data <- read_csv(file = file.path(OUT_DIR, "plot_data_correction_old_vs_new_autocorrelated.csv"))

colour_palette <- viridis(7)
ggplot(filter(plot_comparison_data, between(idx, 130, 150)), aes(x = idx)) +
  ...
  geom_ribbon(aes(x = idx, ymax = median_CI_up_Re_estimate, ymin = median_CI_down_Re_estimate, fill = `Re values`, group = `Re values`),
              alpha = 0.25) +
  coord_cartesian(xlim = c(138,143),
  ylim  = c(1.2, 2.3)) +
...
ggsave(filename = file.path(PLOT_DIR, "Re_correction_old_vs_new_autocorrelated.png"), width = 12, height = 9, units = "cm", dpi = 320)
