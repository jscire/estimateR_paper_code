...
Rt_values_constant <- smooth_incidence(rep(2.5, times = 100), data_points_incl = 8)

Rt_values <- Rt_values_constant
...
for(i in 1:NUM_REPLICATES){
  ...
  truncated_observations <- data.frame(incidence = simulated_observations[truncated_steps:100],
                                       replicate = i)
  simulations <- c(simulations, list(truncated_observations))
}
...
for(i in 1:NUM_REPLICATES){
  print(i)
  truncated_observations <- filter(simulations, replicate == i)

  Re_with_new_leftpadding <- get_block_bootstrapped_estimate(
    incidence_data = truncated_observations$incidence,
    delay = list(delay_short, delay_long),
    ...
    include_index = TRUE,
    data_points_incl = 9) %>% 
    mutate(Rt_type = "new_leftpadding") 
  
  Re_with_old_leftpadding <- get_block_bootstrapped_estimate(
    incidence_data = truncated_observations$incidence,
    delay = list(delay_short, delay_long),
    N_bootstrap_replicates  = 100,
    left_pad_with_zeroes = TRUE,
    combine_bootstrap_and_estimation_uncertainties = TRUE,
    output_Re_only = FALSE,
    include_index = TRUE,
    data_points_incl = 9) %>% 
    mutate(Rt_type = "old_leftpadding") 
  
  comparison_data <- bind_rows(Re_with_new_leftpadding, Re_with_old_leftpadding) %>% 
...
all_bootstrapped_results <- all_bootstrapped_results %>% 
  mutate(idx = idx + truncated_steps)
...
plot_comparison_data$`Re values` <- recode_factor(plot_comparison_data$Rt_type,
                                                  new_leftpadding= "estimateR",
                                                  old_leftpadding= "Huisman et al. pipeline")

write_csv(plot_comparison_data, file = file.path(OUT_DIR, "plot_data_leftpadding_old_vs_new_autocorrelated.csv"))


## Build figure
plot_comparison_data <- read_csv(file = file.path(OUT_DIR, "plot_data_leftpadding_old_vs_new_autocorrelated.csv"),
                                 col_types = list(
                                   idx = col_double(),
...
ggplot(plot_comparison_data, aes(x = idx)) +
  ...
  geom_ribbon(aes(x = idx, ymax = median_CI_up_Re_estimate, ymin = median_CI_down_Re_estimate, fill = `Re values`, group = `Re values`),
              alpha = 0.25) +
    coord_cartesian(xlim = c(truncated_steps - 2,truncated_steps + 5),
                    ylim  = c(1.5, 5.5)) +
...
ggsave(filename = file.path(PLOT_DIR, "Re_leftpadding_old_vs_new.png"), width = 12, height = 9, units = "cm", dpi = 320)